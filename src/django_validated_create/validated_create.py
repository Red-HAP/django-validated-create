...
def build_validated_select(
    model: models.Model,
    record: dict,
    *,
    fetch_related: bool = False,
    from_insert: bool = True,
) -> str:
...
def validate_fetched_against_input(
    fk_fields: Iterable, inp_rec: dict, val_rec: dict
) -> dict:
    for f in fk_fields:
        if f.name in inp_rec:
            fkname = f.name
        elif f.get_attname_column()[-1] in inp_rec:
            fkname = f.get_attname_column()[-1]
        else:
            continue
        vfkname = f.get_attname_column()[-1]

        if val_rec[vfkname] != inp_rec[fkname]:
            related_table = f"{f.related_model._meta.db_table}"
            msg = "is not present in table"
            raise IntegrityError(
                f"Key ({fkname})=({inp_rec[fkname]}) {msg} {related_table}"
            )
...
def execute_sql(statement: str, parameters: dict) -> dict:
    val_rec = None
...
def get_fk_fields(model: models.Model) -> List[models.ForeignKey]:
    return [
        f
        for f in model._meta.concrete_fields
        if isinstance(f, models.ForeignKey)
...
def validated_select(
    ...
    val_sel_sql = build_insert_validated_select(
        model, record, fetch_related=fetch_related, from_insert=False
    )
    val_rec = execute_sql(val_sel_sql, record)

    val_rec = validate_fetched_against_input(
        fk_fields, record, val_rec
    )
...
def validated_create(
    ...
    val_rec = execute_sql(val_ins_sel_sql, record)
    val_rec = validate_fetched_against_input(
        fk_fields, record, val_rec
    )
